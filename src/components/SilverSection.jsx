...
const s = {
  ...
  header: {
    background: 'linear-gradient(135deg, #d8dde2 0%, #a8b0bc 60%, #7a8898 100%)',
...
export default function SilverSection({ prices, categories }) {
  ...
  function calculate() {
    ...
    setResult({ saleMetalPrice, making, finalPrice, purchasePrice, weight: w, categoryName: cat.name, cat })
  }
  ...
  return (
    <section style={s.section}>
      ...
      <div style={s.header}>
        <div style={s.headerTop}>
          <div>
            ...
            <div style={s.headerTitleHindi}>Silver — आज का भाव (प्रति 10 ग्राम)</div>
          </div>
        </div>
        <div style={s.badge}>
          <span style={s.badgeLabel}>चाँदी भाव (Silver Rate)</span>
          <span style={s.badgePrice}>₹ {(prices.silver || 0).toLocaleString('en-IN')}</span>
        </div>
      </div>
      ...
      <div style={s.body}>
        <div style={s.grid}>
          ...
          <div style={s.field}>
            ...
            <input
              ...
              style={s.input}
              onFocus={e => { e.target.style.borderColor = '#4a5a6a'; e.target.style.boxShadow = '0 0 0 3px rgba(106,122,138,0.18)' }}
              onBlur={e => { e.target.style.borderColor = '#b8c0ca'; e.target.style.boxShadow = 'none' }}
            />
          </div>
          ...
          <div style={s.field}>
            <label style={s.label}>
              वस्तु का प्रकार <span style={s.labelSub}>(Category)</span>
            </label>
            {categories.length === 0 ? (
        ...
        {result && <ResultCard {...result} metalType="silver" real10gPrice={prices.silver} />}
      </div>
    </section>
  )
}
